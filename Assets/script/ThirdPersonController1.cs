...
public class ThirdPersonController1 : MonoBehaviour
{
    ...
    public float idleRotationSpeed = 10f; // Speed of idle rotation
    private float idleRotationDirection = 1f; // 1 for right, -1 for left
    public float idleRotationCooldown = 3f; // Time before idle rotation starts
    private float lastMoveTime;
    private float changeDirectionTime = 5f; // Time before switching direction
    ...
    private void HandleIdleCameraRotation()
    {
        if (moveDirection.magnitude < 0.1f) // Player is idle
        {
            if (Time.time > lastMoveTime + idleRotationCooldown) // Wait before starting idle rotation
            {
                cameraTransform.RotateAround(transform.position, Vector3.up, idleRotationSpeed * Time.deltaTime);
            }
        }
        else
        {
            lastMoveTime = Time.time; // Reset idle timer when moving
        }
    }
